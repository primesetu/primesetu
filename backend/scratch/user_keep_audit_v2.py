...
# Normalize function: lowercase and remove underscores
def normalize(name):
    return name.lower().replace('_', '').replace('s9sys', '')

ALLOWED_NORMALIZED = {normalize(t.strip()) for t in USER_KEEP_LIST if t.strip()}

# SMRITI-OS native tables - keeping them separate to ensure they don't get normalized incorrectly
SMRITI_NATIVE = {
