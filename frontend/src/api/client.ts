...
export const api = {
  ...
  inventory: {
    ...
    getForecast: (tier?: string) => apiClient.get('/inventory/forecast', { params: { tier } }).then(r => r.data),
    getNetworkStock: (sku: string) => apiClient.get(`/inventory/${sku}/network-stock`).then(r => r.data),
    createISTRequest: (data: { sku: string, from_store_id: string, qty: number }) => apiClient.post('/inventory/ist/request', data).then(r => r.data),
    listPendingIST: () => apiClient.get('/inventory/ist/pending').then(r => r.data),
  ...
  ho: {
    ...
    confirmCommand: (id: string, managerId: string, pin: string) => apiClient.post(`/ho/confirm-command/${id}`, { manager_id: managerId, manager_pin: pin }).then(r => r.data),
  ...
  config: {
    ...
    getCategories: () => apiClient.get('/config/sysparam/categories').then(r => r.data),
