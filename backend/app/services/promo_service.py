...
class PromoService:
    ...
    async def evaluate_promotions(
        db: AsyncSession, items: List[Dict[str, Any]], bill_total: Decimal
    ) -> Dict[str, Any]:
        """
        ...
        # Note: Shoper9 uses startdate/enddate
        stmt = select(Promomnheader).where(
            and_(Promomnheader.startdate <= now, Promomnheader.enddate >= now)
        )
        ...
        # 2. Fetch Details
        item_stmt = select(Promomnitemlvldiscdtls).where(
            Promomnitemlvldiscdtls.salespromocode.in_(promo_codes)
        )
        bill_stmt = select(Promomnbilllvldiscdtls).where(
            Promomnbilllvldiscdtls.salespromocode.in_(promo_codes)
        )
