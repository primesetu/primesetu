    ...
    for table_name, count in tables:
        ...
        try:
            ...
            while True:
                ...
                query = f'INSERT INTO public."{table_name.lower()}" ({", ".join(keys)}) VALUES ({placeholders}) ON CONFLICT DO NOTHING'
                
                async with pg_pool.acquire() as pg_conn:
                    await pg_conn.executemany(query, records)
                total += len(records)
                print(f"Progress {table_name}: {total}...")
